...
export const POST = withErrorHandler(async (request, { params }) => {
  ...
  // Use the mongoose model directly to update
  ...
  try {
    ...
    if (experience && schedule && customer) {
      await notificationService.sendBookingConfirmation(booking, experience, schedule, customer);
    }
  } catch (err) {
  ...
  return apiSuccess({ message: 'Booking verified and confirmed successfully', booking });
});
